...
export class SeekComponent {
  ...
  appliedJobs: any[] = []; // Array to store applied jobs

  toggleJobApplicationStatus(employee: any) {

    const index = this.appliedJobs.findIndex((job) => job.id === employee.id);
    if (index !== -1) {
  ...
  isJobApplied(employee: any): boolean {
    return this.appliedJobs.some((job) => job.id === employee.id);
  }
